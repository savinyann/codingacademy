<?php
include_once 'Cat.php';
include_once 'Canary.php';
include_once 'Shark.php';

class Animal
{
	public static $numberOfAnimalsAlive = 0;
	public static $numberOfMammals = 0;
	public static $numberOfFishes = 0;
	public static $numberOfBirds = 0;
	const MAMMAL = 0;
	const FISH = 1;
	const BIRD = 2;
	protected $name;
	protected $legs;
	protected $type;


		public static function getNumberOfAnimalsAlive()
	{
		$s = (self::$numberOfAnimalsAlive < 2) ? "" : "s";
		echo "There is currently ".self::$numberOfAnimalsAlive." animal$s alive in our world.\n";
		return self::$numberOfAnimalsAlive;
	}


		public static function getNumberOfMammals()
	{
		$s = (self::$numberOfMammals < 2) ? "" : "s";
		echo "There is currently ".self::$numberOfMammals." mammal$s alive in our world.\n";
		return self::$numberOfAnimalsAlive;
	}


		public static function getNumberOfFishes()
	{
		$s = (self::$numberOfFishes < 2) ? "" : "es";
		echo "There is currently ".self::$numberOfFishes." fish$s alive in our world.\n";
		return self::$numberOfAnimalsAlive;
	}


		public static function getNumberOfBirds()
	{
		$s = (self::$numberOfBirds < 2) ? "" : "s";
		echo "There is currently ".self::$numberOfBirds." bird$s alive in our world.\n";
		return self::$numberOfAnimalsAlive;
	}


	public function __CONSTRUCT($name, $nbr_legs, $type)
	{
		$this->name = $name;
		$this->legs = $nbr_legs;
		$this->type = $type;
		echo "My name is ".$this->getName()." and I am a ".$this->getType()." !\n";
		self::$numberOfAnimalsAlive++;
		switch ($type)
		{
			case 0:
			Animal::$numberOfMammals++;
			break;
			
			case 1:
			Animal::$numberOfFishes++;
			break;
			
			case 2:
			Animal::$numberOfBirds++;
			break;
		}
	}


	public function getName()
	{
		return($this->name);
	}


	public function getLegs()
	{
		return($this->legs);
	}


	public function getType()
	{
		switch ($this->type)
		{
			case 0:
			return("mammal");
			break;

			case 1:
			return("fish");
			break;

			case 2:
			return("bird");
			break;

			default:
			return "inconnu";
			break;
		}
		return($this->type);
	}


	public function setName($name)
	{
		$this->name = $name;
	}
}
?>