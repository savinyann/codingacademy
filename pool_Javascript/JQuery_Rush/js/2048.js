$(document).ready(function()
{
	...
	// Record your input to allow you to play
	...
	// Allow you to undo one move. Will be desactivated soon
	$("#previous").on("click",function()
	{
		if(previous_play != null)
		{
			for(var i = 0; i<= 15; i++)
			{
				$("#"+i.toString()).text(previous_play[i]);
			}
		}
	});
	...
	// Chose what to do when you have played
	...
	// add a random tile on a random place in the field
	...
	function moving_animation(start, end, start_value, end_value)//tile_start, tile_end, value)
	{
		if(start != end)
		{
			var y_start = start%4;  //3
			var x_start = (start-y_start)/4;  //3
			var x_end = end%4;  //1
			var y_end = (end-x_end)/4;  //1
			x_start *= 150;
			y_start *= 150;
			x_end *= 150;
			y_end *= 150;
	...
	// Verify if you have played.
	function check_move(play_field = null)
	{
		if(play_field == null)
		{
			play_field = [];
